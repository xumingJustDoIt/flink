...
object DataSource_MySql {
  ...
  class MySql_Source extends RichSourceFunction[(Int,String,String,String)]{
    ...
    override def run(ctx: SourceFunction.SourceContext[(Int,String,String, String)]): Unit = {
      ...
      //    2. 创建连接
      val connection: Connection = DriverManager.getConnection("jdbc:mysql:///mytest","root","123456")
      //    3. 创建PreparedStatement
      val sql = "select sensor,temp from temperatures"
      val ps: PreparedStatement = connection.prepareStatement(sql)
      //    4. 执行查询
      val resultSet: ResultSet = ps.executeQuery()
      //    5. 遍历查询结果,收集数据
      while(resultSet.next()){
        val sensor= resultSet.getString("sensor")
        val temp = resultSet.getString("temp")

        // 收集数据
        ctx.collect((0,"aa",sensor,temp))
      }
